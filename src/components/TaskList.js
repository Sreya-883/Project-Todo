...
function TaskList() {
  ...
  useEffect(()=>{
  axios.get('http://localhost:4000/TodoTasks')
  .then(response=>{
  ...
  const removeTodo=((id)=>{
    axios.delete(`http://localhost:4000/TodoTasks/${id}`)
    .then(res=>getTasks())
    .catch(err=>console.log(err)); 
  })
  const getTasks = () => {
    axios
      .get("http://localhost:4000/TodoTasks")
      .then((res) => {
        if (res.status === 200) {
          setTask(res.data);
          //clear error message
  ...
  const saveTask= () => {
    ...
    //modify user in DB
    axios
      .put(`http://localhost:4000/TodoTasks/${modifiedTask.id}`,modifiedTask)
      .then((res) => {
        if (res.status === 200) {
          //get recent users
          getTasks();
      ...
      <table className='table table-bordered border border-5 border-white rounded-3'>
            <thead className='bg bg-info '>
                <tr>
                    ...
                    <th width='18%'className='rounded-3 p-3'>Category</th>
                    <th width='18%'className='rounded-3 p-3'>Status</th>
                    <th width='15%'className='rounded-3 p-3'>Edit</th>
                    <th width='15%'className='rounded-3 p-3'>Delete</th>
                </tr>
            </thead>
            <tbody>{
              TodoTask.map(todoTaskObj=>
                <tr className='bg bg-secondary  bg-opacity-25  rounded-3 border-5 border-white'>
                    ...
                    <td className='fw-normal  rounded-3'>{todoTaskObj.category}</td>
                    <td className='fw-normal  rounded-3'>{todoTaskObj.status}</td>
        ...
        </table>:<p className='fs-1 fw-light text-success m-5'>Task List is Empty</p>}
